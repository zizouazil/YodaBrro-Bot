...
client.on('message',async message => {
  ...
  if(message.content.startsWith("!giveaway")) {
    ...
    message.channel.send(`:eight_pointed_black_star:| **Mention the room you Want for the giveaway**`).then(msgg => {
      message.channel.awaitMessages(filter, {
        ...
        if(!room) return message.channel.send(':heavy_multiplication_x:| **I could not Find the Room**');
        room = collected.first().content;
        collected.first().delete();
        msgg.edit(':eight_pointed_black_star:| **Write Giveaway Time**').then(msg => {
          message.channel.awaitMessages(filter, {
            max: 1,
            time: 20000,
            errors: ['time']
          }).then(collected => {
            if(isNaN(collected.first().content)) return message.channel.send(':heavy_multiplication_x:| **You Must a correct Time.. ``Re-write the Command``**');
            duration = collected.first().content * 60000;
            collected.first().delete();
            msgg.edit(':eight_pointed_black_star:| **Finally,What Giveway you want to Provide**').then(msg => {
              message.channel.awaitMessages(filter, {
                max: 1,
                time: 20000,
                errors: ['time']
                ...
                try {
                  ...
                  message.guild.channels.find('name', room).send(giveEmbed).then(m => {
                     let re = m.react(':eight_spoked_asterisk:');
                     setTimeout(() => {
                       let users = m.reactions.get(":eight_spoked_asterisk:").users;
                       ...
                       .setAuthor(message.author.username, message.author.avatarURL)
                       .setTitle(title)
                       .addField('Giveaway Ended!',`The Winner is : ${gFilter}`)
                       .setFooter(message.guild.name, message.guild.iconURL);
                       m.edit(endEmbed);
                     },duration);
                   });
                  msgg.edit(`:heavy_check_mark:| **The Giveaway is Made**`);
                } catch(e) {
                  msgg.edit(`:heavy_multiplication_x:| **I could not Make the Giveaway\nReason: Loss in Permissions**`);
                  console.log(e);
                }
              });
            });
